...
const models=["User", "AlumniProfile"];
const payload={format:'riverside-community-v1',project:'alumni-map',exported_at:new Date().toISOString(),tables:{},media:[]};
try {
  ...
  try {
    if(names.size) {
      ...
      for(const name of names) {
        const source=await fs.realpath(path.join(sourceRoot,name));
    ...
    console.log(JSON.stringify({sha256:crypto.createHash('sha256').update(raw).digest('hex'),counts:Object.fromEntries(Object.entries(payload.tables).map(([k,v])=>[k,v.length])),media:payload.media.length},null,2));
