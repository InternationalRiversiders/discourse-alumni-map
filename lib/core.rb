...
module DiscourseAlumniMap
  ...
  %w[Command Event Audit Legacy Media Reaction Report Comment].each do |name|
    ...
    klass.table_name = "river_alumni_map_#{name.underscore.pluralize}"
    klass.table_name = "river_alumni_map_media" if name == "Media"
    const_set(name, klass)
  end
  module Access
    def self.admin?(user)
      user && user.active? && !user.suspended? && (user.admin? || user.in_any_groups?(SiteSetting.alumni_map_admin_groups.split("|").map(&:to_i)))
    end
    def self.member?(user)
      user && user.active? && !user.suspended? && (admin?(user) || user.in_any_groups?(SiteSetting.alumni_map_allowed_groups.split("|").map(&:to_i)))
    end
    def self.check!(user, admin: false)
      raise Discourse::InvalidAccess unless SiteSetting.alumni_map_enabled && (admin ? admin?(user) : member?(user))
    end
  end
  module Shared
    ...
    def self.lock(key)
      number = Digest::SHA256.digest("river_alumni_map:#{key}").unpack1("q>")
      ...
      Event.where(notification_id: nil).order(:id).limit(100).each do |event|
        event.with_lock do
          ...
          n = Notification.create!(user_id: event.user_id, notification_type: Notification.types[:custom], skip_send_email: true,
            data: { river_app: "alumni_map", river_text: event.text, river_path: event.path, river_icon: "map-location-dot", message: "alumni_map", display_username: "", topic_title: event.text }.to_json)
          event.update!(notification_id: n.id)
        end
      rescue => e
        Rails.logger.warn("alumni_map notification #{event.id}: #{e.class}")
      end
    end
    ...
    def self.media_urls(ids) = Array(ids).map { |id| "/alumni-map/media/#{id}" }
    ...
    def self.react(user, item, value)
      ...
      if value.to_i == 1 && item.respond_to?(:user_id) && item.user_id && item.user_id != user.id
        notify(item.user_id, "你的内容收到新的赞", key: "like:#{item.class.name}:#{item.id}:#{user.id}")
      end
      {}
    end
    def self.report(user, item, reason)
      Report.create_or_find_by!(user_id: user.id, target_kind: item.class.name.demodulize, target_id: item.id) { |r| r.reason = text(reason, 500) }
      {}
    end
    ...
    def self.comment(user, item, data)
      ...
      c = Comment.create!(user_id: user.id, target_kind: kind, target_id: item.id, parent_id: parent&.id, body: text(data["body"], 2000), anonymous: bool(data["anonymous"]), media_ids: media_ids(user,data["media_ids"]), rating: data["rating"].presence)
      recipients = [parent&.user_id, item.respond_to?(:user_id) ? item.user_id : nil].compact.uniq - [user.id]
      recipients.each { |uid| notify(uid, "你的内容有新的回复", key: "comment:#{c.id}:#{uid}") }
      { id: c.id }
    end
  ...
  class MainController < ::ApplicationController
    requires_plugin "discourse-alumni-map"
    skip_before_action :check_xhr, only: [:index, :media, :map_frame, :export]
    before_action :enabled!
    rescue_from Error, ArgumentError do |error|
      render_json_dump({ errors: [error.message] }, status: 422)
    end
    def index
      Access.check!(current_user) unless false
    ...
    def export
      ...
      data={profile:Profile.find_by(user_id:current_user.id)&.attributes,legacy:UserLifecycle.archive(current_user.id)}
      send_data(JSON.pretty_generate(data), type:"application/json", disposition:"attachment", filename:"alumni-card.json")
    end
    def locations
      Access.check!(current_user)
      response.headers["Cache-Control"] = "private, no-store"
      render_json_dump(Locations.options(params[:country], params[:province]))
    end
    def map_frame
      Access.check!(current_user)
      response.headers["Cache-Control"] = "private, no-store"
      # AMap needs eval and inline scripts. Confine those permissions to an
      # opaque-origin sandbox that cannot access the forum DOM or cookies.
      response.headers["Content-Security-Policy"] = "upgrade-insecure-requests; default-src 'none'; script-src 'unsafe-inline' 'unsafe-eval' https://*.amap.com; style-src 'unsafe-inline' https://*.amap.com; img-src data: blob: https://*.amap.com https://*.autonavi.com; connect-src https://*.amap.com https://*.autonavi.com; worker-src blob:; frame-ancestors 'self'; sandbox allow-scripts"
      render html: File.read(File.expand_path('../public/map-frame.html', __dir__)).html_safe, layout: false
    end
    def mutate
      Access.check!(current_user)
      RateLimiter.new(current_user, "alumni_map-write", 40, 1.minute).performed!
    ...
    def upload
      Access.check!(current_user)
      raise Error, "这是只读快照，请到演示站测试编辑" if SiteSetting.alumni_map_read_only
      RateLimiter.new(current_user, "alumni_map-upload", 20, 1.hour).performed!
      ...
      raise Error, "压缩后图片最大 1MB" if bytes.bytesize > 1.megabyte
      ...
      render_json_dump({ id: item.id, url: "/alumni-map/media/#{item.id}" })
    rescue Vips::Error
    ...
    private
    def enabled!
      raise Discourse::NotFound unless SiteSetting.alumni_map_enabled
    end
  end
end
module ::Jobs
  class DiscourseAlumniMapTick < ::Jobs::Scheduled
    every 1.minute
    def execute(args)
      return unless SiteSetting.alumni_map_enabled
      DistributedMutex.synchronize("alumni_map-tick") do
        DiscourseAlumniMap::Service.tick if DiscourseAlumniMap::Service.respond_to?(:tick)
        DiscourseAlumniMap::Shared.deliver
      end
    end
  end
end
...
module DiscourseAlumniMap
  ...
  module Ui
    ...
    def self.shell(user, query, tabs, **rest)
      { title: "校友地图", intro: "在世界各处，遇见同路的校友。", view: query["view"].presence || tabs.first[0], member: Access.member?(user), admin: Access.admin?(user),
  ...
  module Shared
    def self.interaction(user, operation, data)
      ...
      when 'react','report','comment','delete_own','moderate'
        ...
        if operation=='moderate'
          ...
          notify(item.user_id,'你的内容有新的管理处理，请查看详情',key:"moderate:#{item.class.name}:#{item.id}:#{item.updated_at.to_f}") if item.user_id
          return {}
        end
    ...
    def self.interaction_ui(item,user,comments: false,anonymous: false,images: false)
      ...
      if Access.member?(user)
        ...
        if comments
          fields=[Ui.field('body','写下回复',nil,type:'textarea',required:true)]
        ...
        forms<<Ui.form('举报','report',[Ui.field('reason','举报理由',nil,required:true)],data,button:'提交举报')
        actions<<Ui.action('删除我的内容','delete_own',data) if item.respond_to?(:user_id) && item.user_id==user.id
