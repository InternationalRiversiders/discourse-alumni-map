...
import LocationPicker from "./alumni-map-location";
...
export default class extends Component {
  ...
  @action async upload(e) {
    ...
    try {
      for (const file of Array.from(e.target.files).slice(0, 6)) {
        ...
        const result = await ajax("/alumni-map/upload", {
          type: "POST",
          data,
          processData: false,
          contentType: false,
  ...
  @action async submit(event) {
    ...
    this.busy = true;
    this.error = "";
    if (event.target.querySelector(".alumni-location select:disabled")) {
      this.busy = false; this.error = "请等待地区列表加载完成后保存"; return;
    }
  ...
  get groups() {
    ...
    let definitions = [];
    if (this.args.form.operation === "save") {
      definitions = [
        [
          "城市与名片",
          "选择城市即可，不需要填写坐标或详细住址。位置选填。",
          ["nickname"],
        ],
        [
          "在校时光",
          "共同的校园，是最自然的话题。",
          ["admission_year", "graduation_year", "school", "major"],
        ],
        [
          "现在的你",
          "工作、爱好，或一段简单的自我介绍。",
        ...
        [
          "公开设置",
          "公开后，仅有校友地图访问权限的论坛成员可见；联系方式也属于名片内容。",
          ["published", "show_username"],
        ],
      ];
    }
      ...
      {{#each this.groups key="title" as |group|}}
        ...
        >
          ...
          <div class="river-fields">{{#each group.fields key="name" as |field|}}
            ...
            {{/each}}{{#if group.location}}<LocationPicker @fields={{@form.fields}} />{{/if}}</div>
        </fieldset>
      {{/each}}
